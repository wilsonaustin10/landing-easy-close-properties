...
export async function POST(request: Request) {
  try {
    ...
    if (data.submissionType === 'business_acquisition') {
      ...
      // Send to GHL as primary destination
      const ghlSuccess = await ghlIntegration.submitBusinessLead(data);
      if (!ghlSuccess) {
        console.error('Failed to submit business lead to GHL');
      } else {
        console.log('Successfully sent business lead to GHL');
      }
      ...
      */
      ...
      // Send to GHL as primary destination
      const ghlSuccess = await ghlIntegration.submitPropertyLead(data);
      if (!ghlSuccess) {
        console.error('Failed to submit property lead to GHL');
      } else {
        console.log('Successfully sent property lead to GHL');
      }
